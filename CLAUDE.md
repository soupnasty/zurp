...
- **Category mapper** (`categories.ts`): 4-tier classification — merchant name match (high) → Plaid detailed category (medium) → Plaid primary category (low; `GENERAL_MERCHANDISE` splits online/in-store via `payment_channel`) → `other`. Ambiguous mega-merchants (Amazon/Walmart/Target, `deferToPlaid` flag) yield to Plaid's `FOOD_AND_DRINK_GROCERIES` signal per-transaction. Uses 26-category taxonomy separate from the 8-category spending system.
