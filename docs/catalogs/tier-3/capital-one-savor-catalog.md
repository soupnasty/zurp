...
```benefit_key: dining_entertainment_scope
Dining encompasses all restaurant and bar purchases. Entertainment includes movie theaters, concert venues, sporting events, and similar attractions.
```
...
Return protection reimburses eligible items if merchandise is unavailable, returned, or refuses to accept return from a merchant. Coverage up to $300 per item and $1,000 per year.
```

### Cash Back & Redemption

```benefit_key: cash_back_only
This is a cash back card with no transfer partners. Points are redeemed only as statement credits or direct deposits to a bank account.
```

```benefit_key: welcome_bonus_cash
$200 statement credit after $500 in purchases within the first 3 months.
```
...
```benefit_key: no_transfer_partners
Unlike premium travel cards, Capital One Savor does not offer transfer partners to airlines or hotel loyalty programs. Redemption is limited to cash back only.
```

```benefit_key: no_lounge_access
Lounge access (Priority Pass, Amex Lounge, etc.) is not included with this card.
```

```benefit_key: no_travel_insurance
Comprehensive trip protection insurance (trip cancellation, trip delay, lost luggage) is not included with this card.
```

```benefit_key: no_purchase_protection_extended
Extended warranty and additional purchase protection benefits beyond return protection are not included.
```
...
|----------|-----------|-------|
| Dining | 3% | All restaurants and bars |
| Entertainment | 3% | Movie theaters, concerts, events |
...
|---------------|---------|-------|
| Return Protection | Item refund/return reimbursement | $300 per item, $1,000 yearly |
...
| Purchase Protection | Limited to return protection | See above |
