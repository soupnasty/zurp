# Southwest Rapid Rewards Priority Visa Card Catalog

*Last verified: 2026-08-13*

## Card Overview

| Attribute | Details |
|-----------|---------|
| Card Name | Southwest Rapid Rewards Priority Visa Signature |
| Issuer | Chase Bank |
| Network | Visa Signature |
| Annual Fee | $229 |
| Card Type | southwest_priority |
| Primary Use Case | Frequent Southwest Airlines travelers seeking Companion Pass pathway, priority boarding, and family travel benefits |

## Benefit Catalog

### Points & Earning Structure

```benefit_key: earning_southwest_flights
4x Rapid Rewards points per $1 spent on Southwest Airlines flight bookings made directly through Southwest.com or Southwest ticket counters. This is the highest earning category on the card.
```

```benefit_key: earning_dining
2x Rapid Rewards points per $1 spent at all dining establishments including restaurants, bars, cafes, and food delivery services. This category encourages everyday spending on dining when not traveling.
```

```benefit_key: earning_gas_stations
2x Rapid Rewards points per $1 spent at gas stations, added in the July 2025 card refresh alongside the move to 4x on Southwest flights.
```

```benefit_key: earning_other_purchases
1x Rapid Reward point per $1 spent on all other eligible purchases not listed in the premium earning categories, establishing baseline consistent earning across remaining spending.
```

### Annual Travel Credits

```benefit_key: southwest_travel_credit_annual
DISCONTINUED: The $75 annual Southwest travel credit was removed in the July 2025 card refresh and ended 12/31/2025. It previously applied toward eligible Southwest Airlines purchases including airfare, baggage fees, seat selections, early check-in, and other airline incidentals. Retained here for historical usage tracking only.
```

### Companion Pass Pathway Benefits

```benefit_key: companion_pass_qualifying_points
10,000 qualifying Companion Pass points earned annually from card spending and bonuses, contributing toward the Companion Pass threshold. Rapid Rewards points earned through the card count toward Companion Pass qualification.
```

```benefit_key: companion_pass_anniversary_bonus
7,500 additional Companion Pass qualifying points awarded each cardmember anniversary, providing automatic progress toward Companion Pass status without requiring corresponding spending.
```

### Boarding Privileges

```benefit_key: priority_boarding_group_5
Priority boarding Group 5 for the primary cardholder on all Southwest Airlines flights, ensuring boarding after A and B groups but before general boarding, providing better seating options.
```

```benefit_key: priority_boarding_companions_8
Priority boarding Group 5 extended to up to 8 companions traveling on the same reservation as the primary cardholder, allowing family groups to maintain seating proximity on Southwest flights.
```

### Upgraded Boarding Benefits

```benefit_key: upgraded_boarding_annual
DISCONTINUED January 27, 2026: The 4 complimentary Upgraded Boardings per year were eliminated when Southwest moved to assigned seating. Replaced by unlimited Extra Legroom seat upgrades (see below). Retained here for historical reference only.
```

```benefit_key: extra_legroom_seat_upgrades
Unlimited complimentary Extra Legroom seat upgrades within 48 hours of departure, subject to availability, plus Group 5 boarding. Introduced January 27, 2026 as the replacement for the discontinued Upgraded Boardings benefit.
```

### Baggage Benefits

```benefit_key: free_checked_bag_cardholder
First checked bag waived for the primary cardholder on all Southwest Airlines flights, representing a savings of $35 per flight. Southwest allows two checked bags per traveler when both are free.
```

```benefit_key: free_checked_bag_companions_8
First checked bag fee waived for up to 8 companions traveling on the same itinerary as the primary cardholder, provided companions are ticket holders on the same reservation. This creates significant family travel value.
```

### In-Flight Benefits

```benefit_key: inflight_beverage_wifi_discount
25% rebate on in-flight drinks and WiFi purchases on Southwest flights, providing incremental savings on airport connectivity and beverage purchases. Rebates are tracked and applied as statement credits.
```

### Welcome Bonus

```benefit_key: welcome_bonus_rapid_rewards
50,000 Rapid Rewards points welcome bonus upon meeting the minimum spending requirement of $1,000 in eligible purchases within the first 3 months of account opening.
```

### International Features

```benefit_key: no_foreign_transaction_fee_southwest
No foreign transaction fees assessed on purchases made outside the United States, facilitating international travel and vacation spending without currency conversion markup.
```

### Travel Insurance Benefits

```benefit_key: travel_accident_insurance_southwest
Travel accident insurance provides coverage for serious accidents occurring during covered trips, including emergency evacuation and transportation assistance for injuries sustained during airline travel.
```

```benefit_key: baggage_delay_insurance
Baggage delay reimbursement provides $100 per day up to 3 days for essential purchases if baggage is delayed more than 12 hours, assisting with necessities while awaiting luggage.
```

```benefit_key: lost_luggage_reimbursement
Lost luggage coverage reimburses the cardholder for lost or damaged baggage up to $3,000 per trip, providing essential protection for travel valuables and necessities.
```

### Purchase & Warranty Protection

```benefit_key: purchase_protection_southwest
Purchase protection reimburses cardholders for eligible items damaged or lost within 120 days after purchase, up to $500 per claim and $50,000 annual aggregate account limit.
```

```benefit_key: extended_warranty_southwest
Extended warranty protection extends the manufacturer's warranty by an additional one year on eligible items originally covered by manufacturer's warranty of three years or less.
```

### Car Rental Coverage

```benefit_key: collision_damage_waiver_secondary
Secondary collision damage waiver provides supplemental rental car damage coverage when the primary insurance declines coverage, subject to policy limitations and exclusions.
```

## Points Multipliers Summary

| Category | Multiplier | Notes |
|----------|-----------|-------|
| Southwest Flights | 4x | Direct bookings on Southwest.com |
| Dining | 2x | All restaurants and food establishments |
| Gas Stations | 2x | Added in July 2025 refresh |
| All Other Purchases | 1x | Base earning rate |
| Companion Pass Progress | 17,500/yr | 10K spend + 7.5K anniversary bonus |

## Companion Pass & Elite Benefits

| Benefit | Details |
|---------|---------|
| Companion Pass Progress | 10,000 annual points + 7,500 anniversary = 17,500/year toward status |
| Family Value Multiplier | 8 companions with free bags + Group 5 boarding = significant family value |
| Annual Credits | None — $75 travel credit discontinued 12/31/2025; Upgraded Boardings replaced by unlimited Extra Legroom upgrades Jan 27, 2026 |
| Two-Person Travel | Companion Pass allows companion to fly free on paid tickets |

## Insurance & Protection Coverage

| Coverage Type | Benefit | Limit |
|---------------|---------|-------|
| Trip Cancellation | NOT INCLUDED | N/A |
| Travel Accident | Accident coverage | Standard terms |
| Baggage Delay | Essential item reimbursement | $100/day up to 3 days |
| Lost Luggage | Baggage reimbursement | $3,000 per trip |
| Purchase Protection | Item damage/loss reimbursement | $500/claim, $50,000/account |
| Extended Warranty | Warranty extension | 1 year additional coverage |

## Detailed Benefit Analysis

The Southwest Rapid Rewards Priority Visa Signature card targets dedicated Southwest loyalists and family travel groups through a combination of boarding privileges, companion benefits, and Companion Pass pathway acceleration. At $229 annually, the card is offset by unlimited Extra Legroom seat upgrades, free checked bags, and the pathway value of 17,500 annual Companion Pass qualifying points. Note: the July 2025 refresh discontinued the $75 annual travel credit (dead 12/31/2025), and the 4 annual Upgraded Boardings were eliminated January 27, 2026.

The card's primary strength lies in its unmatched family travel benefits: free checked bags for the cardholder and up to 8 companions combined with priority boarding Group 5 for the same group creates significant value for families and traveling groups. The unlimited Extra Legroom seat upgrades (bookable within 48 hours of departure, subject to availability) provide tactical flexibility for important trips requiring premium seating.

The Companion Pass pathway is critical: earning 17,500 qualifying points annually (10,000 from spend + 7,500 anniversary bonus) means cardholders need only $42,500 in annual card spending to reach the 110,000 point Companion Pass threshold—far more achievable than the standard $30,000 in ticket purchases required to earn Companion Pass. Once achieved, Companion Pass allows a designated companion to fly free on all paid Southwest tickets for a full year.

The 4x earning on Southwest flights plus 2x on dining and gas stations adds meaningful redemption velocity for everyday spending, particularly valuable when combined with the flexible Southwest award booking system that allows point redemptions on any flight with no blackout dates.

Critical weakness: the card notably excludes trip cancellation insurance, a standard inclusion on most premium travel cards. Additionally, the limited checked bag companion benefit (only applies to those on same reservation) restricts flexibility for coordinating travel within family groups traveling on different itineraries.

Best suited for households with multiple Southwest travelers and annual combined spending exceeding $40,000 who can exploit the Companion Pass pathway and family boarding benefits. Less valuable for solo travelers or those who split airline loyalty.

---

**card_type:** southwest_priority
**Last Updated:** August 2026
